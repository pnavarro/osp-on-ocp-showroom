...
run_deployment() {
    ...
    if [[ -n "${CRED_REGISTRY_USERNAME:-}" ]]; then
        ...
        # Verify registry injection worked
...
main() {
    ...
    if [[ -n "$credentials_file" ]]; then
        ...
        if [[ "$debug_credentials" == "true" ]]; then
            ...
            print_status "  CRED_SUBSCRIPTION_MODE: ${CRED_SUBSCRIPTION_MODE:-unset}"
            if [[ "${CRED_SUBSCRIPTION_MODE:-}" == satellite ]]; then
                print_status "  CRED_SATELLITE_URL: $CRED_SATELLITE_URL"
                print_status "  CRED_SATELLITE_ORG: $CRED_SATELLITE_ORG"
                print_status "  CRED_RHC_ACTIVATION_KEY: $([ -n "${CRED_RHC_ACTIVATION_KEY:-}" ] && echo "SET" || echo "EMPTY")"
            else
