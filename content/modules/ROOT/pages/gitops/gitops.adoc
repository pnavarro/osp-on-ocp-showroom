...
* xref:prepare-external-secrets-operator.adoc[Install External Secrets Operator (Kubernetes provider)]
* xref:secure.adoc[Configure Secure Access with Vault and External Secrets]
...
Make sure you uncheck "Copy the main branch only" checkbox.
...
----
cd
git clone git@github.com:$your_github_id/rhoso-gitops.git rhoso-gitops
cd rhoso-gitops
----
